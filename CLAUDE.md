...
- 9+1 节骨架(模板 `_build/templates/source.html`,样板 `library/learning/practice-effectively.html`):1 来源卡 `.src-card` → 2 一句话核心(费曼法)`.keypoints` → 3 论证脉络 3–6 段 `.narr` → 4 关键概念术语 `.explain` ul(与 preset 一致)→ 5 中英原句精选 3–5 条 `.quote`(每句 ≤25 词)→ 6 追问与反例 → 7 知识连线(`<ul class="links">`,每条 `<li>` 带 `.lk-tag` 标签「本库课页」/「外部」(外部加 `class="ext"`),相对路径链到本库课页 + 外部;不要用裸 `<ul>`,字体会变衬线)→ 8 自测题 `.quiz #quiz`(题目同时写进 meta.quiz,供总目录「考我」抽题)→ 9 我的输出 `.my-output textarea#my-output`(自动保存)→ **10 原文全文**(nav 加 `<div class="part">附 · 原文</div>`,section `#s10`「原文全文 · Full Transcript」:首段 `.narr` 注来源/作者/年份/链接/授权,然后全部原文逐句中英对照放 `<div class="para"><p class="en">` + `.sent .se/.sz`,按话题分 8–12 段;一句不漏,字数必须与字幕对齐)。Shirley 2026-09-24 定:来源没废话(TED-Ed 等)就放完整原文;书要放章节原文,但仓库 Public、版权原文不能上线,Shirley 2026-09-24 定:仓库保持 Public(免费),书只放精选段落,不放整章。学习科学研究备忘在 `_build/LEARNING_SCIENCE.md`,改版式/功能前先看它。
