...
- 9+1 节骨架(模板 `_build/templates/source.html`,样板 `library/learning/practice-effectively.html`;**v2 学习版式,Shirley 2026-09-24 定**):1 来源卡 `.src-card` → **先想再读** `.pretest`(3 个问题 + `textarea[data-save="pretest"]`)→ 2 一句话核心(费曼法)`.keypoints`,整块包在 `.reveal`(`.reveal-btn` 「对照原版」+ `.reveal-body`,默认折叠,先自己说)→ 3 论证脉络 3–6 段 `.narr` → 4 关键概念术语 `.explain` ul(与 preset 一致)→ 5 中英原句精选 3–5 条 `.quote`(每句 ≤25 词)→ 6 追问与反例 → 7 知识连线(`<ul class="links">`,每条 `<li>` 带 `.lk-tag` 标签「本库课页」/「外部」(外部加 `class="ext"`);不要用裸 `<ul>`,字体会变衬线)→ 8 自测题 `.quiz #quiz`:每题 `<div class="qa" data-i="N">` 含 `.q` 问题、`textarea[data-save="quiz-N"]`(先自由回忆写/说)、`.qa-show` 看答案、`.a` 答案、`.rate` 四个按钮 `data-r=0..3`(忘了/模糊/记得/很熟);题目同时写进 meta.quiz → 9 我的输出 `.my-output`:三个固定提示 `textarea[data-save="out-own|out-example|out-limit"]`(用自己的话说/举一个自己的例子/它在哪不适用),老页面保留 `#my-output` → **10 原文全文**(nav 加 `<div class="part">附 · 原文</div>`,section `#s10`「原文全文 · Full Transcript」:首段 `.narr` 注来源/作者/年份/链接/授权 + 一句「这里放全文是用来核对的,学习请用挖空复述」,然后全部原文逐句中英对照放 `<div class="para"><p class="en">` + `.sent .se/.sz`,按话题分 8–12 段;一句不漏,字数必须与字幕对齐)。**s2–s7 每节标题下一行 `<p class="cue">` 提示问题**(工具栏「只看提示」= `body.cues-only` 只留提示,自己回忆)。**挖空复述(Shirley 2026-09-24 定的核心学法)**:重点词包 `<span class="cz">`,最重要的包 `<span class="cz big">`(大圈),同一句里英文和中文要**成对、同序**标(点一个两边一起揭开);s2/s3/s4/s5/s10 都要标,全页约 60–80 个概念、大圈 15–30 个;工具栏「挖空复述」= `body.cloze`,词变空胶囊,中英都藏,点开显示,底部浮条 `#cloze-bar`(全部揭开/全部盖上/已揭开 n/N/退出),N 按概念数(中英一对算一个)。来源页 `<body class="src-page">`(17px/1.7 行距/640px 栏)。Shirley 2026-09-24 定:来源没废话(TED-Ed 等)就放完整原文;仓库保持 Public(免费),书只放精选段落,不放整章。学习科学研究备忘在 `_build/LEARNING_SCIENCE.md`,改版式/功能前先看它。
...
- localStorage 键:任何 `textarea[data-save="X"]` 自动存到 `<key>-X`(`#my-output` ≡ `<key>-output`);`__sched`(间隔复习总表,对象:键 `<pageKey>#<i>` 自测题 / `<pageKey>@<word>` 生词,值 `{due:"YYYY-MM-DD",ivl:1|3|7|14|30,n,r:0-3}`;评分 r=0 回到 1 天、r=1 不变、r=2 进一级、r=3 跳两级;从未评过的按 1 天起算);`__quiz_log`(`[{key,i,r,ok,t}]`,最多 500 条)、`__rev_<key>`、`__last_read_<key>`(课页和来源页共用)、`<key>-vocab`(生词本,抽 `known:false` 的词)。这些都在本机浏览器,不走 Gist 同步。总目录:每个课程页顶部横幅「今日复习 n 条」;大脑库栏「今日复习」= 一张卡一题的自由回忆(写/说 → 看答案 → 忘了/模糊/记得/很熟),自测题和生词卡(中文面朝上、翻面)跨页跨学科混排,一轮最多 20 条;「考我」随机抽题。
